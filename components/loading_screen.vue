...
// The root is the portfolio landing now (Aetherveil moved to /aetherveil), and
// landing samples are plain pages too: a sky-blue village intro over a dark
// portfolio is wrong branding and delays first paint.
const isLandingPage = computed(() => route.path === '/' || route.path.startsWith('/samples'));
// Only the game keeps the short village intro that covers Phaser init.
const isGamePage = computed(() => route.path.startsWith('/aetherveil'));

// The journal has its own per-story backdrop, so skip the village intro there.
const loading = ref(!isJournalPage.value && !isClientsPage.value && !isLandingPage.value);
