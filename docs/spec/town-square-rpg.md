...
| 2026-06-13 | **j9: world rebuilt from the overworld design map** ([concept-map-j9.png](./concept-map-j9.png), authoritative for layout): 80×60 @ 32px, single Phaser scene. The j8 world was deleted wholesale and recreated to match: Keep + cascade NE with the river running south to the sea (two bridges), Cathedral of Whisperleaf-on-the-Hill, Atelier north of the wall, west lane (Vaults + 6 trade shops + odd houses + Quiet Grove), walled square (bell tower, bazaar, well, Mayor), Hearthlight Inn + Great Gate, windmill spawn crossroads, Clock Tower underpass on the east road, stilt house, stables + paddock, wheat fields + scarecrows, Beacon on its shore field, dock on the south beach. Systems (fishing, findings, audio, interiors, persistence) carried over intact. |
| 2026-06-12 | **Implementation complete (j8).** Valley shipped at 64×52 (town kept its j1 coordinates; new land east + south). All 14 regions live: grove (swing, meditation stone, petals), waterfall + pool + hidden coin, river + stone bridge + east meadow, quest board, market stalls ×3, residential lane (cat lady ×5 cats, inventor 7-click rune, painter easel modals, hermit riddles, music hut with 4 WebAudio-synth melodies — zero audio files), windmill, clock-tower underpass (camera dim + chime), beach + 5 shells + crab + net-mender, fishing minigame (weighted 6-species, mythic keeper), Beacon rebuilt as shore lighthouse. Systems: collision grid, findings (10 collectibles) + HUD, synth SFX + mute, sunset-on-idle, bird flocks, virtual d-pad (pointer:coarse), reduced-motion. Castle kept from j1d as the north landmark (spec deviation, deliberate). Audio uses generated WebAudio cues instead of .ogg assets (K.5 resolved). |
